#!/usr/bin/env python3
"""Smoke-test the kamae-model-translator plugin package without external dependencies."""
...
def check_manifest_skill_paths(errors: list[str]) -> None:
    ...
    for plugin in data.get("plugins", []):
        ...
        if plugin.get("name") == "kamae-model-translator" and not skills:
            fail(errors, f"{marketplace.relative_to(ROOT)}: kamae-model-translator entry missing skills")
        for entry in skills:
            path = (ROOT / entry).resolve()
...
def prose_text(text: str) -> str:
    """Remove fenced code blocks and inline code spans so link checks do not
    match type-application syntax like decode[Dto](value)."""
...
def main() -> int:
    ...
    print("Checked JSON manifests, skill frontmatter, Markdown links, manifest skill paths, and Python scripts.")
    return 0
